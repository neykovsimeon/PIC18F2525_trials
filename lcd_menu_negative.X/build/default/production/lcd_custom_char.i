...
const char dlineR[] = {30,17,30,20,18,0,31,31};
...
for (i=0; i<8; i++){
while(BusyXLCD());
WriteDataXLCD (Pattern[i]);
}

}

void switch_to_custom_char(const char *text_to_convert) {
...
case 2: {
initial_ddramaddress = (0x14 + x);
break;
}
case 3: {
initial_ddramaddress = (0x54 + x);
break;
}
default: {
initial_ddramaddress = (0x00 + x);
...
my_dline_chars[dline_index].dline_used = 1;
...
for (lcd_i = 20 - lcd_offset - x - 1; lcd_i; lcd_i--) {
while(BusyXLCD());
